...
/// <summary>
/// Component to handle player gameplay - this is mostly from DevulTj's (sbox-template.fps) PlayerControllerMechanic
/// </summary>
public abstract class PlayerMechanic : EntityComponent<Player>
{
	...
	protected Controller Controller => Entity.Controller;
	...
	/// <returns></returns>
	public bool TrySimulate( Controller controller )
	{
		var before = IsActive;
	...
	/// </summary>
	/// <returns></returns>
	protected virtual bool ShouldStart() => TimeUntilCanStart;
	...
	public override string ToString() => $"{Name}: IsActive({IsActive})";
	...
	private static float ApproximateWallHeight( Vector3 startPos, Vector3 wallNormal, float maxHeight, float maxDist,
		int precision, out Vector3 tracePos, out float absoluteHeight )
	{
		...
		var foundWall = false;
		for ( var i = 0; i < Math.Min( precision, MaxWallTraceIterations ); i++ )
		{
			startPos.z += step;
			currentHeight += step;
