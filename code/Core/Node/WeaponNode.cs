...
public abstract partial class WeaponNode : Entity, IInventoryItem
{
	public partial class Connector : Entity
	{
		...
		[Net] public WeaponNode WeaponNode { get; set; }
	...
	/// <summary>
	/// <see cref="Core.Player"/> that owns this entity
	/// </summary>
	protected Player Player => InUse ? (Player)Owner.Owner : (Player)Owner;

	[Net] public IList<Connector> Connectors { get; set; } = new List<Connector>();

	[Net] private WeaponNode Previous { get; set; }

	// this is because BaseNetworkable exchanging sucks (well, now it's an Entity. But I don't want it to be!)
	[Net] private string PreviousConnectorId { get; set; }
	...
	protected float? GetValue( string identifier, float? dimension )
	{
	...
	/// <exception cref="UnknownConnectorException">If no connector found</exception>
	public void SetConnector( string identifier, WeaponNode weaponNode )
	{
		...
		if ( connector == null )
		{
			throw new UnknownConnectorException( identifier );
		}
		...
		bool useMaxEnergy = false )
	{
		...
		if ( previousOutput == null )
		{
			...
			var absolute = providedConnector?.ConnectorAttribute?.EnergyOutAbsolute;
		...
		foreach ( var connector in Connectors.OrderBy( v => v?.ConnectorAttribute?.Order ?? 99 ) )
		{
			...
			if ( connector.ConnectorAttribute.EnergyOutAbsolute == 0.0f &&
			...
			else
			{
				...
				usage = MathF.Min( result, connector.ConnectorAttribute.EnergyOutAbsolute );
			}
	...
	[Net] public Guid Uid { get; private set; } = Guid.NewGuid();
}
